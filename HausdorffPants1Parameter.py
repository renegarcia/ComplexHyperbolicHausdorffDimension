...
def Hausdorff(levmax, t, d):
    ...
    centros = [
        [-1.00 / m.cos(t * m.pi), 0.00],
        [1.00 / m.cos(t * m.pi) * e1, 0.00],
        [(1.00 / m.cos(t * m.pi)) * e2, 0.00],
    ]
        ...
        for j in range(1, 4):
            for iold in range(num[lev - 2], num[lev - 1]):
                if j == inv[tag[iold - 1] - 1]:
                    continue
                z = float(
                    abs(tagpoints[iold - 1][0] - centros[j - 1][0]) ** 4.00
                    + (
                        tagpoints[iold - 1][1]
                        - centros[j - 1][1]
                        - 2
                        * (
                            np.conjugate(tagpoints[iold - 1][0]) * centros[j - 1][0]
                        ).imag
                    )
                    ** 2
                )
                u = complex(
                    abs(tagpoints[iold - 1][0] - centros[j - 1][0]) ** 2
                    + 1j
                    * (
                        tagpoints[iold - 1][1]
                        - centros[j - 1][1]
                ...
                tagpoints[inew - 1][0] = centros[j - 1][0] + (
                    -1.00 * radios[j - 1] ** 2
                ) * (tagpoints[iold - 1][0] - centros[j - 1][0]) * u * (z ** (-1.00))
                tagpoints[inew - 1][1] = centros[j - 1][1] + (z ** (-1.00)) * (
                    -1.00
                    * (radios[j - 1] ** 4)
                    * (
                        tagpoints[iold - 1][1]
                    ...
                    * (
                        (
                            np.conjugate(
                                (tagpoints[iold - 1][0] - centros[j - 1][0]) * u
                            )
                            * centros[j - 1][0]
                        ).imag
                    )
        ...
        for i in range(0, N):
            for j in range(0, N):
                if i == j:
                    ...
                    z = (abs(radios[i]) ** 4) / (
                        abs(w[cont][0] - centros[i][0]) ** 4
                        + (
                            w[cont][1]
                            - centros[i][1]
                            - 2 * (np.conjugate(w[cont][0]) * centros[i][0]).imag
                        )
                        ** 2
                    )
                ...
                if wordsN[i][1:levmax] == wordsN[j][0 : levmax - 1]:
                    for k in range(0, 3):
                        if wordsN[i][0] == words[k]:
                            z = (abs(radios[k]) ** 4) / (
                                abs(w[cont][0] - centros[k][0]) ** 4
                                + (
                                    w[cont][1]
                                    - centros[k][1]
                                    - 2
                                    * (np.conjugate(w[cont][0]) * centros[k][0]).imag
                                )
                                ** 2
                            )
                            T[i][j] = abs(z) ** -1.00
...
n = int(input("Provide the Number maximal of groups "))
k = int(input("Maximal depth on the Word-tree"))
